...
# Q3
# a
path = "C:/Users/Erezd/OneDrive/Desktop/Age_and_Time.csv"
df = pd.read_csv(path)

x = df['Age']
y = df['Time']
plt.scatter(x , y, label='Ampirical data')
plt.xlabel("Age")
plt.ylabel("Time")
plt.xlim(0,35)

b_LS = np.mean(y) / np.mean(x)
# add least squares line :
plt.plot(x, b_LS * x + 0 , 'b', label='least squares line')
plt.legend()
plt.show()
...
'''
r_midgami = np.cov(x,y)[0][1] / (np.std(x) * np.std(y))
print(r_midgami ** 2)
'''
